...
export default function IdentityForm({ identity, onSave, onCancel }) {
  ...
  const hasAutoGenerated = useRef(!!identity?.tagline); // skip if editing existing

  const autoGenerateAll = async (currentForm) => {
    if (hasAutoGenerated.current || !currentForm.name?.trim()) return;
    hasAutoGenerated.current = true;
    setAutoGenerating(true);
    const prompts = {
      tagline: `Write a single punchy tagline (max 12 words) for a ${currentForm.role_label || 'Freelancer'} named "${currentForm.name}". Return ONLY the tagline.`,
      bio: `Write a compelling 3-4 sentence professional bio in first person for "${currentForm.name}", a ${currentForm.role_label || 'Freelancer'} with ${currentForm.communication_tone || 'professional'} tone. Return ONLY the bio.`,
      email_signature: `Write a concise professional email signature (3-4 lines) for "${currentForm.name}", ${currentForm.role_label || 'Freelancer'}. Return ONLY the signature text.`,
      proposal_style: `Write 4-5 bullet-point proposal writing instructions for "${currentForm.name}", a ${currentForm.role_label || 'Freelancer'}. Format as "- instruction". Return ONLY the bullet points.`,
    };
    const fields = ['tagline', 'bio', 'email_signature', 'proposal_style'];
    const results = await Promise.allSettled(
      fields.map(f => base44.integrations.Core.InvokeLLM({ prompt: prompts[f] }))
    );
    setForm(prev => {
      const updated = { ...prev };
      fields.forEach((f, i) => {
        if (results[i].status === 'fulfilled' && results[i].value && !prev[f]) {
          updated[f] = results[i].value.trim();
        }
      });
      return updated;
    });
    setAutoGenerating(false);
    toast.success('✨ MISSION auto-filled your identity fields');
  };
