 ...
 *
 * PHASE 2 FIXES:
 * 1. Fixed 3 trailing-dot links: '/VeloIdentityHub.' → '/VeloIdentityHub'
 * 2. Onboarding detection now checks active identity: goals?.onboarded || !!activeIdentity
 * 3. Identity Hub module card shows identity count / active name instead of null
 * 4. Added identities query for stat display
 */
...
import {
  ...
  TrendingUp, Radio, ChevronRight, Cpu, Settings, Power,
  Rocket, Lock, AlertTriangle, Zap, Users, Coins,
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  // ── PHASE 2 FIX: query identity count for module card stat ──
  ...
  // ── PHASE 2 FIX: also consider active identity as onboarded ──
  const isOnboarded = goals?.onboarded === true || !!activeIdentity;
  ...
  const MODULES = [
    {
      ...
      color: '#fbbf24',
      stat: activeTasks > 0 ? activeTasks : isAutopilotOn ? 'ON' : 'OFF',
      statLabel: activeTasks > 0 ? 'active tasks' : 'status',
      active: isAutopilotOn,
      aiLabel: 'AUTOPILOT',
    },
    {
      to: '/Discovery',
    ...
    {
      // ── PHASE 2 FIX: removed trailing dot from route ──
      to: '/VeloIdentityHub',
      icon: Users,
      title: 'IDENTITY HUB',
      subtitle: 'Personas, KYC, credentials, and platform account management',
      color: '#818cf8',
      // ── PHASE 2 FIX: show active identity name or identity count instead of null ──
      stat: activeIdentity?.name || identities?.length || 0,
      statLabel: activeIdentity?.name ? 'active' : 'identities',
    ...
    },
    {
      ...
      color: '#10b981',
      stat: `$${todayEarnings.toFixed(0)}`,
      statLabel: 'today',
      active: todayEarnings > 0,
      aiLabel: 'VAULT',
    },
    {
      to: '/DigitalResellers',
    ...
    },
    {
      ...
      color: '#f97316',
      stat: pendingInterventions,
      statLabel: 'pending',
      active: pendingInterventions > 0,
    },
  ];
      ...
      {warpTarget && (
        <div
          ...
          style={{
            ...
            animation: 'warp-jump 0.6s ease-in forwards',
          }}
        >
          <div className="text-center">
            <div className="font-orbitron text-xs tracking-[0.4em] text-cyan-400 mb-2 animate-pulse">
              WARPING TO
            </div>
            <div className="font-orbitron text-3xl text-white tracking-widest">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div
                ...
                style={{ background: 'linear-gradient(to bottom, #00e8ff, #ff2ec4)' }}
              />
              <div>
                <h1
                  className="font-orbitron text-3xl md:text-4xl font-black text-white tracking-wider"
                  style={{ textShadow: '0 0 30px rgba(0,232,255,0.4)' }}
                >
                  VELO AI
                </h1>
                <p className="text-[10px] text-slate-600 font-mono tracking-widest">
                  COMMAND HUB · SIX DEPARTMENTS
                </p>
              </div>
            </div>
        ...
        {!isOnboarded && (
          ...
          <Link to="/VeloIdentityHub">
            <div
              className="mb-4 px-5 py-4 rounded-2xl flex items-center justify-between cursor-pointer transition-all"
              style={{
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-orbitron text-xs tracking-widest text-violet-300 block">
                    SETUP REQUIRED — COMPLETE YOUR VELO AI ONBOARDING
                  </span>
                  <span className="text-[10px] text-slate-500">
                    Identity · KYC · Autopilot · Banking — 5 minutes to activate the full platform
                  </span>
                </div>
              </div>
        ...
        {pendingInterventions > 0 && (
          <Link to="/Execution">
            <div
              className="mb-4 px-5 py-3 rounded-2xl flex items-center justify-between cursor-pointer transition-all"
              style={{
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-orbitron text-xs tracking-widest text-orange-400">
                  AUTOPILOT BLOCKED — {pendingInterventions} intervention
                  {pendingInterventions !== 1 ? 's' : ''} require your input
                </span>
              </div>
        ...
        {/* ── STATUS BANNER ── */}
        <div
          className="mb-8 px-5 py-3 rounded-2xl flex items-center justify-between"
          style={{
        ...
        >
          <div className="flex items-center gap-3">
            <Radio
              className={`w-4 h-4 ${isAutopilotOn ? 'text-cyan-400 animate-pulse' : 'text-slate-600'}`}
            />
            <div>
              <span
                className="font-orbitron text-xs tracking-widest"
        ...
        <div className="mb-2">
          <p className="font-orbitron text-[10px] tracking-[0.3em] text-slate-600 mb-4">
            CORE DEPARTMENTS
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <div className="px-5 py-3 border-b" style={{ borderColor: 'rgba(0,232,255,0.1)' }}>
              <span className="font-orbitron text-xs tracking-widest text-cyan-400/70">
                RECENT EXECUTION
              </span>
            </div>
            <div className="p-4 space-y-2 max-h-56 overflow-y-auto">
              {tasks.slice(0, 8).length === 0 ? (
                <div className="text-center py-6 text-slate-600 text-xs">
                  No tasks yet — activate Autopilot to begin
                </div>
              ) : (
                        ...
                        style={{
                          background:
                            task.status === 'completed'
                              ? '#10b981'
                              : task.status === 'executing' || task.status === 'running'
                              ? '#00e8ff'
                              : task.status === 'failed'
                              ? '#ef4444'
                              : '#f59e0b',
                    ...
                    <div className="flex items-center gap-2">
                      {task.revenue_associated > 0 && (
                        <span className="text-xs font-mono text-emerald-400">
                          +${task.revenue_associated.toFixed(2)}
                        </span>
                      )}
          ...
          <div
            ...
            style={{
              background: 'rgba(10,15,42,0.65)',
              border: '1px solid rgba(255,46,196,0.12)',
              backdropFilter: 'blur(20px)',
            }}
          >
            <div className="px-5 py-3 border-b" style={{ borderColor: 'rgba(255,46,196,0.1)' }}>
              <span className="font-orbitron text-xs tracking-widest text-pink-400/70">
                QUICK ACTIONS
              </span>
            </div>
            <div className="p-4 space-y-2.5">
              {[
                ...
                {
                  to: '/Execution',
                  label: `Resolve Interventions${pendingInterventions > 0 ? ` (${pendingInterventions})` : ''}`,
                  color: '#f97316',
                  icon: AlertTriangle,
                },
